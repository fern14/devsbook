...
class UserDaoMysql implements UserDAO
{
  ...
  private function generateUser($array, $full = false)
  {
    ...
    if ($full) {
      $ur = new UserRelationDaoMysql($this->pdo);
      $postDaoMysql = new PostDaoMysql($this->pdo);
      // Seguidores
      $u->followers = $ur->getFollowers($u->id);
      foreach ($u->followers as $key => $follower) {
        $newUser = $this->findById($follower);
        $u->followers[$key] = $newUser;
      }

      // Quem eu sigo
      $u->following = $ur->getFollowing($u->id);
      foreach ($u->following as $key => $followin) {
        $newUser = $this->findById($followin);
